...
export default function InputComponent() {
  ...
  const fetchInputs = async () => {
    await axios.get(`${process.env.NEXT_PUBLIC_API_REST_URL}/document_sources`, {
      params: { run_id: runId },
      headers: {
  ...
  const fetchProcessingLogs = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_REST_URL}/processing_logs`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  const removeDocument = async (id: string) => {
    ...
    if (doc?.type === 'pdf') {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_API_REST_URL}/remove_uploaded_file`, null, {
        params: { 
          run_id: runId,
          file_name: doc.title,
        },
  ...
  const processFiles = async () => {
    ...
    const responseForFiles = await axios.post(`${process.env.NEXT_PUBLIC_API_REST_URL}/upload-files`, formData, {
      params: { 
        run_id: runId,
      },
      headers: {
  ...
  const processUrls = async () => {
    ...
    const response = await axios.post(`${process.env.NEXT_PUBLIC_API_REST_URL}/add-urls`, formData, {
      params: {
        run_id: runId,
      },
      headers: {
  ...
  const processDocuments = async () => {
    ...
    await axios.get(`${process.env.NEXT_PUBLIC_API_REST_URL}/launch_run`, {
      params: { 
        run_id: runId, 
      },
      headers: {
